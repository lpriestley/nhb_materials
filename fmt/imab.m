...
% function imab(data, clims, bSep, noCols)
%
...
%
% bSep is boolean option to separate each image from its neighbours with
% zeros
%
...
if size(data,4) > 20
    disp('Dataset more than 20 time points, try plot_all instead...')
    return
end
...
if bSep && size(data,3)>1
    ...
    if size(data,4)==1
        ...
        for iR = 1:noRows
            for iC = 1:noCols
                ...
                if iPlots(iR,iC) > dims(3)
                    ...
                    hndl=imagesc(data(:,:,iPlots(iR,iC)),clims);aet;
                end
            end
        end
        ...
        for iR = 1:noRows
            for iC = 1:noCols
                ...
                hndl=imagesc(data(:,:,iC,iR),clims);aet;
            end
        end
    ...
    if size(data,4)==1
        ...
        if isempty(clims)
            hndl=imagesc(outIm);aet
        else
            hndl=imagesc(outIm,clims);aet;
        end
    else
        if isempty(clims)
            hndl=imagesc(imcat(imcat(data,2,3),1,4,0));aet;
        else
            hndl=imagesc(imcat(imcat(data,2,3),1,4,0),clims);aet;
        end
    end
